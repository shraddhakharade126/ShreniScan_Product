...
import { inr, products, type Product } from "@/lib/kalakart-data";
import { getActiveCatalog, getAllDrafts } from "@/lib/draft-store";
...
function Home() {
  ...
  const [catalogList, setCatalogList] = useState<Product[]>(products);

  useEffect(() => {
    async function loadData() {
      const active = await getActiveCatalog();
      const drafts = await getAllDrafts();
      const draftProducts: Product[] = drafts.map((d) => ({
        id: d.id,
        name: d.title || "In-Progress Draft",
        price: d.finalPrice || d.analysis?.suggestedPrice || 1499,
        rating: 5.0,
        stock: 1,
        status: "Draft",
        image: d.studioImage || d.rawImage,
        craft: d.analysis?.craftType || "Artisan Craft",
      }));
      setCatalogList([...draftProducts, ...active]);
    }
    loadData();
  }, []);

  const mine = catalogList.filter(
    (p) =>
